...
### M18 外部评审整改：数据竞争 / 错误语义 / 执行路径收敛（已提交前修复）

评审发现的三个关键问题全部修复并加回归测试：
1. **响应头共享切片数据竞争**（生产阻断）：`ctJSON/ctText` 共享 []string 被
   中间件原地修改污染全请求——改为 `setCT` 每响应独立切片；回归测试
   `TestHeaderSliceIsolation` + race 全绿；
2. **序列化错误语义不一致**（糖入口 500 vs 通用路径 200 空响应）：渲染器增加
   **无状态 Prepare 阶段**（`Prepare(v) ([]byte, error)` + `WritePrepared`，
   渲染器实例跨请求共享故不缓存状态），序列化在提交响应头前完成；JSON/XML/
   HTML 模板的执行错误统一走 500 错误管道；回归测试覆盖四形态；
3. **两档性能差异**：Prepare 重构后两档收敛（文本 66.3 vs 64.7ns、JSON
   182.9 vs 182.1ns，均在同量级），README 明确执行路径与语义保证。
- 自评估取舍：修复 1 放弃"冻结头零分配"（换每响应 1 次小分配），正确性优先；
  修复 2 的第一版（渲染器缓存 bytes.Buffer）在 race 下暴露共享状态缺陷，
  改为返回字节的无状态设计后通过。
...
3. 修复二：顺带发现**注册性能 bug**——旧实现每次 Mount 全树 sortByWeight
   （5000 路由注册 O(N² log N) 数秒），改为注册期 O(1)、排序移到启动收尾：
   `App.SortRoutes()` 显式调用（`App.Serve` 自动调用），惰性 sync.Once 兜底
   保留——12 万路由实测排序约 22ms，首个请求不再承担（稳态约 110ns）；
